...
import { CMS_KEY_ES, type SiteContent } from "@/lib/cms";

// Only these fields from SiteContent need Gemini translation.
// Everything else (phone numbers, emails, URLs, feature flags, dates) stays as-is.
interface TranslatableSlice {
  announcement: { text: string; type?: string };
  seniorMenu: {
    note: string;
    meals: Record<string, { headline: string; full: string[] }>;
  };
  marketSchedule: {
    note: string;
    transportation: string;
    stops: Record<string, { time: string; location: string }[]>;
  };
...
function extractTranslatableSlice(content: SiteContent): TranslatableSlice {
  return {
    announcement: {
      ...
      type: content.announcement.type,
    },
    seniorMenu: {
      note: content.seniorMenu.note,
      meals: content.seniorMenu.meals,
    },
    marketSchedule: {
      note: content.marketSchedule.note,
      transportation: content.marketSchedule.transportation,
      // Location names are kept as-is — they're proper Oklahoma town names
...
3. DO NOT translate proper nouns: CADC, Head Start, Early Head Start, Advantage, VITA, ChildPlus, Red River, Red River Transit, Red River Transportation, Upstash, Vercel.
...
8. Staff titles should be translated formally (e.g. "Executive Director" → "Director Ejecutiva/o").
9. The audience is Spanish-speaking residents of rural Southwest Oklahoma. Use clear, accessible language — not formal academic Spanish.`;
...
export async function POST(req: Request) {
    ...
    const geminiRes = await fetch(
      `https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash:generateContent?key=${apiKey}`,
      {
        ...
        body: JSON.stringify({
          ...
          generationConfig: {
            temperature: 0.1,        // low temp = consistent, literal translation
  ...
  // Merge translated slice back into full content structure
  // Everything not in the slice (features, boardDocs, timestamps, etc.) stays English
  const esContent: SiteContent = {
    ...
    staff: content.staff.map((s, i) => ({
      ...
      // name stays English — proper nouns
    })),
